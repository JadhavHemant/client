...
// ✅ FIX: Define API URL at module level (outside component)
const API_BASE_URL = (() => {
    // For Vite projects
...
const Company = () => {
    ...
    // ✅ FIX: Wrap fetchCompanies in useRef to make it stable
    const fetchCompanies = useRef(async (limit = 10, offset = 0, search = '') => {
        setLoading(true);
        console.log(`📡 Fetching companies: limit=${limit}, offset=${offset}, search="${search}"`);
    ...
    const openEditModal = (company) => {
        ...
        // ✅ FIX: Use API_BASE_URL instead of process.env
        if (company.LogoUrl) {
            setLogoPreview(`${API_BASE_URL}${company.LogoUrl}`);
        }
        setModalMode('edit');
        setSelectedCompany(company);
        setShowModal(true);
